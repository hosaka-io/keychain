...
(defrecord Database [db-conf datasource]
  component/Lifecycle
  ...
  (start [this]
    (do
      ...
      (let [ds (make-datasource db-conf)
        ...
        (.setSchemas flyway (into-array ["keychain"]))
...
(defn new-database [{:keys [db-url db-user db-password]}]
  (map->Database {:db-conf {:username db-user :password db-password :jdbc-url db-url}}))
